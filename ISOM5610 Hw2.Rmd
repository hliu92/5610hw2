...
bluestem.CO.1[,2] <- bluestem.CO.1[,2]-1 
...
Error <- matrix(0,nrow=4,ncol=3)
colnames(Error) <- c("Mean Squared Error","Absolute Error","Absolute Error Percentage")
rownames(Error) <- c("add.fit.2","mtp.fit.1","co.fit.1","mtp.fit.4")
...
## co.fit.1 
bluestem.CO.2 <- as.data.frame(lapply(bluestem[,c(13,2)], as.numeric))
bluestem.CO.2[,2] <- bluestem.CO.2[,2]-1 
co.pd.1 <-  (co.fit.1$coefficients[1]/(1-rho)+
               bluestem.CO.2$Promotion*co.fit.1$coefficients[2])*bluestem$Index
dif <- bluestem$Sales-co.pd.1
...
Error[4,1] <- crossprod(dif,dif)/249
Error[4,2] <- sum(abs(dif))/249
Error[4,3] <- sum(abs(dif/bluestem$Sales))/249
